...
describe("fold", () => {
  ...
  it("returns error if invalid", () => {
    ...
    expect(fold(onNone, onError, onValue)(invalid(errors))).toEqual(
      onError(errors),
    )
  })
...
describe("map", () => {
  it("return the value as a VALID type, the localValue otherwise", () => {
    ...
    expect(map(a_remapping_function)(invalid(any_value))).toEqual(
      invalid(any_value),
    )
    expect(map(a_remapping_function)(absent())).toEqual(absent())
    expect(map(a_remapping_function)(valid(any_value))).toEqual(
      valid(a_remapping_function(any_value)),
    )
  })
})
